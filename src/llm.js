...
function buildSystemPrompt(text, title, fromApp, timestamp) {
  return [
    ...
    buildOutputFormats(),
    buildPaymentMethodMapping(),
    buildExamples()
  ].join('\n\n');
}

function buildPromptIntroduction() {
...
function buildOutputFormats() {
  return '- You output JSON body for "transaction":\n' +
'{\n' +
'  "type": "transaction",\n' +
...
function buildPaymentMethodMapping() {
  return '2. Mapping Table for payment method\n' +
